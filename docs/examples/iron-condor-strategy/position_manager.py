"""
Position Manager for Iron Condor Options Strategy

This module handles position management for Iron Condor options trades
...
class IronCondorPositionManager(PositionManagerProtocol):
    """Position manager for Iron Condor options strategy."""
    ...
    def __init__(self, *, algorithm: "Algorithm") -> None:
        ...
        self.max_positions = 1  # Only one Iron Condor at a time
        ...
        # Iron Condor specific tracking
        self.iron_condor_trades = 0
        self.premium_collected = 0.0
        self.premium_paid = 0.0
        self.net_premium = 0.0
        self.profit_zone_hits = 0  # How often price stays in profit zone

    async def initialize(self) -> None:
        """Initialize the Iron Condor position manager."""
        print("🦅 Initializing Iron Condor Position Manager")
        print(f"   Max Loss: ₹{self.max_loss}")
        print(f"   Position Size: {self.position_size} lot")
        print(f"   Max Positions: {self.max_positions}")
        print("   Strategy: Iron Condor Options")
        print("=" * 50)
    ...
    async def can_open_position(self, symbol: str) -> bool:
        ...
        # Check if we already have positions (Iron Condor is one trade)
    ...
    async def _update_performance_metrics(self) -> None:
        ...
        try:
            ...
            # Update Iron Condor specific metrics
            self.iron_condor_trades = len(
                [t for t in self.trade_history if t.get("option_type")]
            )

            # Check if current price is in profit zone (simplified)
            # In real implementation, check against strike prices
            self._check_profit_zone()

        except Exception as e:
            print(f"❌ Error updating performance metrics: {e}")

    def _check_profit_zone(self) -> None:
        """Check if current price is in profit zone (simplified)."""
        # In real implementation, check if current Nifty price
        # is between the inner strikes (profit zone)
    ...
    def get_performance_stats(self) -> dict:
        ...
        return {
            ...
            "current_loss": self.algorithm.net_pnl.loss,
            "iron_condor_trades": self.iron_condor_trades,
            "premium_collected": self.premium_collected,
            "premium_paid": self.premium_paid,
            "net_premium": self.net_premium,
            "profit_zone_hits": self.profit_zone_hits,
        }
    ...
    def get_iron_condor_summary(self) -> dict:
        """Get Iron Condor specific summary."""
        return {
            "strategy_name": "Iron Condor",
            "strategy_type": "Neutral Options Selling",
            "total_iron_condor_trades": self.iron_condor_trades,
            "premium_collected": self.premium_collected,
            "premium_paid": self.premium_paid,
            "net_premium": self.net_premium,
